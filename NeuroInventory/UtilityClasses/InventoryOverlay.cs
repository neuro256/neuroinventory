...
namespace NeuroInventory.UtilityClasses
{
    internal class InventoryOverlay : AbstractOverlay
    {
        private readonly InventoryCardRenderer businessCardRenderer = new InventoryCardRenderer();
    ...
    internal class InventoryCardRenderer : AbstractRenderer
    {
        ...
        public void DrawBusinessCard(Graphics g, Rectangle itemBounds, object rowObject, ObjectListView olv, OLVListItem item)
        {
            ...
            if(olv.SelectedObjects.Count > 1)
            {
                ...
                using (Font font = new Font("Tahoma", 8))
                {
                    ...
                    decimal sum = 0;
                    foreach (object obj in olv.SelectedObjects)
                    {
                    ...
                    g.DrawString(sum.ToString(), font, this.TextBrush, textBoxRect, fmt);
